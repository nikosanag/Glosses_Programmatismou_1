...
worker(_,[],M,M).
worker(A,[0|Tail],L,S):- worker(A,Tail,L,S).
worker(A,[1|Tail],L,S):- worker(A,Tail,L,S).
...
minbases([Head|Tail],Result):-
    Head>0,
    multiple_suitable_bases(Head,List_of_suitable_bases),
    worker(Head,List_of_suitable_bases,0,Min),
    minbases(Tail,Rest),
